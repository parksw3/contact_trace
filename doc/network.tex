...
We develop and test statistical methods for both spatial and temporal corrections of generation intervals,
and show that correcting observed intervals for the effect of temporal censoring but \emph{not} for spatial effects provides a spatially informed ``effective'' generation-interval distribution, which correctly links $r$ and $\RR$.
...
Assuming that the individual properties are independent of risk of infection, the population-level kernel is given by integrating over these individual variations \jdnew:
\begin{equation}
K(\tau) = \int K_a (\tau) f(a) da,
\end{equation}
...
\jd{I haven't thought about this for a long time. dA actually represents something pretty confusing (a chunk of the \emph{distribution} across the space). Probably easier to change than explain, but we do need a distribution now.}
...
Throughout this paper, we assume that there is a constant per-pair contact rate $\lambda$. \jddel{, following \citep{trapman2016inferring}}
...
\jd{Not quite sure what Daniel is going for here: this equation depends on having an $r$, which does depend on exponentiality; also it is not how the number is ``defined'' at all. Not even sure if it's how it's estimated, except on a star network. This should be rewritten and not wait for reviewers.}
\begin{equation}
...
We therefore expect that starting from observed generation intervals and correcting for the right-censoring effect, will allow us to estimate an ``effective'' generation interval that accurately reflects dynamics of spread \jdnew. 
...
Similarly, using the intrinsic generation-interval distribution overestimates the reproductive number because it fails to account for local spatial effects; the egocentric distribution corrects for this only partially \jdnew.
Both population-level and individual-level methods provide estimates of reproductive number that are consistent with the empirical estimates, which we define as the average number of secondary cases generated by the first 100 infected individuals, but the individual-level method is more precise \jdnew.
...
estimates based on the egocentric generation-interval distribution \eref{conditional} address this problem only partially \jdnew, as they do not account for indirect spatial effects. 
...
Accounting for the egocentric effect allowed us to link the results by \cite{trapman2016inferring} to established results \jdnew\ based on generation intervals.
