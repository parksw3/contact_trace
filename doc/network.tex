...
Assuming a constant per-pair contact rate of $\lambda$ for any pair, we obtain the following egocentric generation-interval distribution (see Methods for derivation):
\begin{equation}
...
Hereafter, we will refer to the temporally corrected distribution as the ``effective'' generation-interval distribution $\tsub{g}{eff}(t)$.
...
\begin{equation}
\tsup{g}{exp}(\tau) \propto c_t(\tau) \exp(r\tau),
\end{equation}
...
Out study demonstrates that generation intervals measured through contact tracing capture key characteristics (e.g., contact structure) an outbreak; 
these implicit features allow us to infer the ``correct'' generation-interval distribution, which accurately represents how disease spreads in a population. 
...
\swp{TODO:}
\subsection{Egocentric ...}

\subsection{Population-level method}

\subsection{Individual-level method}
