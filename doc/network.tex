...
As an epidemic progresses, an infector is less likely to infect individuals later in time due to decrease in susceptibles and the distribution of forward generation intervals will be shorter than the intrinsic distribution \citep{kenah2008generation, tomba2010some, champredon2015intrinsic}.
...
A susceptible individual is thus relatively more likely to be infected by a newly infected individual, and the distribution of backward generation intervals will be shorter than the intrinsic distribution \citep{champredon2015intrinsic, britton2019estimation}.
...
Whereas forward and backward generation intervals represent infections that are one generation away from the focal cohort, these aggregated generation intervals represent infections across multiple cohorts. 
In this case, the time of infection of a cohort can range anywhere from the beginning of an epidemic to the most recent infection.
...
Realized generation intervals also vary across space.
...
In order to integrate over the aspect space $A$, we have to assume that (1) theoretical distribution of individual kernel properties are known exactly and (2) the individual properties are independent of risk of infection; 
therefore, it only makes sense to define this integral for simple epidemic models for homogeneously mixing population, such as the Susceptible-Infected-Recovered (SIR) model (or its extensions).
In practice, it is unlikely that averaging across an empirical distribution will give us the correct population-level kernel, especially when an epidemic is driven by a few highly infectious individuals.
For brevity, we use this notation to distinguish the difference between the individual- and the population-level kernels.
...
g(\tau) = \frac{K(\tau)}{\RR_0}.
\end{equation}
These definitions of the basic reproductive number and the intrinsic generation-interval distribution assume homogeneous mixing, as discussed above.
...
Then, the infectiousness of an individual $\RR_a = \int K_a(s) ds$ is the product of the number of acquaintances $N_a$ and the contact rate $\lambda$ and the dilution term is equal to the reciprocal of the number of acquaintances: $\delta_a = 1/N_a$.
...
The individual-level method assumes that the initial exponential growth phase of an epidemic can be approximated by a nonhomogeneous Poisson process.


to derive a likelihood for the observations from each individual:
\begin{equation}
\begin{aligned}
