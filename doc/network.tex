...
\swp{I might have an idea... let me think about this a bit more...}
\begin{figure}[!pbth]
...
\tsub{g}{eff}(\tau) \propto c_t(\tau) \exp(r\tau).
\end{equation}
In order to do so, we first fit a gamma distribution to the observed generation-interval distributions; specifically, we esimate the mean and the shape parameter of a gamma distribution by maximum likelihood.
\swp{Come back}
