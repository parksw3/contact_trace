...
\end{equation}
\swp{Need to say something about trapman!!!}

The difference between conditional and intrinsic generation-interval distribution can be best demonstrated by simulating stochastic infection processes on a small tree network, in which a single infected individual is connected to multiple susceptible individuals, with artificially high $\RR$ (Figure ??? to be made).
