...
Generation intervals, defined as the time between when an individual is infected and when that individual infects another person, provides a link between two key quantities that describe an epidemic: the reproductive number, $\RR$, and the rate of exponential growth, $r$.
Often, generation intervals are measured through contact tracing by identifying who infected whom;
we study how these observed intervals vary both spatially and temporally.
Temporal variation in generation intervals is caused by the censoring in the observation process through contact tracing, whereas spatial variation in generation intervals reflects the patterns of spread of disease.
Therefore, correcting for temporal variation, but not for spatial variation, provides an ``effective'' generation-interval distribution, which correctly links $r$ and $\RR$.
...
\swp{I meant to say that because the backward GI is observed GI is observed at one given time, all infectees share the same infection time. Is this clearer?}
While an epidemic is ongoing, these measurements are ``censored'': we don't know what happens after the observation time.
These censored observations can be thought of as a weighted average of backward generation-interval distributions (weighted by incidence); \dc{weighted by what?} \swp{equation 8}
...
This means that contacts made late in an individual's infection are more likely to be ineffective due to infectious contacts that were made earlier (and so the contactee will have been infected already) 
\dc{Although I get you try to explain in simple terms what's going on with ``masked'', I think it's not really clear: masked by what?  is it that subsequent contacts in the neighbourhood do not count given they were infected earlier?}.
\swp{Tried to clarify}
...
An individual-level intrinsic infection kernel $K_a(\tau)$ describes the rate at which secondary infections are expected to be caused by an infected individual $a$.
\dc{is it clearer to specify the kernel is about infections?}
\swp{Yes,}
...
\swp{I think that's clearer too.}

\dc{again, for me it's the intrinsic infectiousness averaged over the population, that's the $\int da$ thing. I stop from now on complaining about this ;)}
\swp{I think figure 1 shows that we're averaging across infectioussness as well as the time distribution. We also say above that ``Individuals may vary in their prognosis (e.g., duration of a latent and an infectious period) and their infectiousness''}
...
\swp{They were (and are) in equation 8. I made it explicit.}
...
\dc{shouldn't it be ``censored'' instead of ``observed mean''??}
\swp{I think observed in OK. Observation through contact tracing is what causes censoring.}
...
\swp{This is a notation issue... equality only holds during the exponential period. TODO: talk to Jonathan about notation.}
Second, decreasing number of susceptibles over the course of an epidemic makes long infections less likely to occur, and the realized \dc{censored?} \swp{no, realized; there is no observation process in the second phenomenon} intervals will always be shorter \citep{champredon2015intrinsic}.
...
When an epidemic is growing exponentially, the right-censoring causes the observed generation-interval to be inverse-exponentially weighted \eref{exp}; \dc{the previous sentence doesn't read well; forgot words?} \swp{Yes, forgot words. Not sure if ``inverse-exponentially weighted'' is clear, however.}
...
\swp{Yes, but they also have a package called epigrowthfit which is based on Ma et al. Maybe in acknowledgements?}
