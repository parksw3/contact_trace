...
\swp{Got rid of forward stuff with some justification (?)}
...
Unlike the intrinsic generation-interval distribution, the observed generation-interval distributions depend on when and how they are measured \citep{kenah2008generation, nishiura2010time, tomba2010some, champredon2015intrinsic, britton2019estimation}. 
Here, we describe how backward generation-interval distributions change over time (see \cite{kenah2008generation, nishiura2010time, tomba2010some, champredon2015intrinsic} for discussion on forward generation intervals); backward measurements better reflect real outbreak scenarios where contact tracing is performed by identifying infectors of infected individuals.
...
Whereas backward (and forward) generation intervals represent infections that are one generation apart from a cohort, these aggregated generation intervals represent infections across multiple cohorts; 
...
the observed (censored) generation-interval is a weighted distribution averages over backward generation-interval distributions up until the observation time.
...
\dc{I'm not sure which paragraph you are mentionning. If it's the one preceding, well, see my comment above. If it's the one following, yes, keep it. 
More generally, I would keep as much definitions as it is necessary for the average reader to understand. Even if many thing said up to now are in other papers (that you cite), it really helps to have everything re-defined and interpreted in this context. This is relatively deep stuff, so anything that helps is welcome, at the risk of being slightly repetitive with previous work. }
