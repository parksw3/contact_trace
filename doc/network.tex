...
There are important distinctions to be made when defining generation-interval \emph{distributions} at the population-level: \emph{intrinsic} generation-interval distributions describe the expected time distribution of infectious contacts made by a primary case,
...
While the shape of intrinsic generation-interval distribution is often assumed to be invariant across time, the shape of the realized generation-interval distribution depends on when and how intervals are measured \citep{kenah2008generation, nishiura2010time, tomba2010some, champredon2015intrinsic, britton2019estimation}.
...
Forward generation intervals can contract over the course of an epidemic due to susceptible depletion \citep{kenah2008generation, nishiura2010time, tomba2010some, champredon2015intrinsic}.

Forward generation intervals are also affected by spatial structure.
...
As a result, forward generation intervals will have shorter mean than the intrinsic generation-interval distribution.
...
In practice, realized generation intervals are often measured by aggregating available information from contact tracing.
...
We further predict the effective generation-interval distribution to be equivalent to the \emph{initial} forward generation-interval distribution, which describes the renewal process of infection during the exponential growth rate.
