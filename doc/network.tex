...
\begin{equation}
i(t) = S(t) \int K(s) i(t-s) ds = \RR(t) \int g(s) i(t-s) ds,
\end{equation}
...
\swp{Reviewer 2 was not happy with this description. Come back.}
...
\swp{They all affect R. I guess we weren't clear enough. Also, the point of our paper is that we can use 
Euler-Lotka equations as long as we know which distribution to use. I tried to clarify these points.}
...
\swp{we showed that these effects shorten the mean observed generation intervals, which in turn will reduce the estimate of the reproduction number.}
...
When temporal correction is performed early in an outbreak, during the exponential growth phase, the effective distribution should incorporate egocentric and local spatial effects but not the global effects; we expect this distribution to correctly link $r$ and $\RR$ \swp{through the Euler-Lotka equation}. 
...
\swp{Here, we assume that \emph{all} generation intervals (no under-reporting) are observed without error.}
...
The first person in the queue becomes infected (and therefore $S_i$ is decreased by 1), and the current time is updated to infection time of this individual. 
