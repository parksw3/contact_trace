...
These observations allow us to make the following prediction: removing the time-censoring bias from observed generation intervals will give us a corrected distribution that contains information about the population structure; using this corrected distribution should allow us to correctly infer $\RR$ from $r$.
...
\end{equation}
As shown in \cite{champredon2015intrinsic},
%% \dc{I added the reference for the reader who wants to see how exactly this is derived}
the backward generation-interval distribution, $b_t(\tau)$, describes a distribution of infection that occurred $\tau$ time units before the reference time $t$ and is proportional to $i(t-\tau) g(\tau)$:
...
\swp{TODO: ask JD}
\jd{I like $b_0$ for the initial backward. We can then use $c_0$ for the initial censored, I guess.}
\begin{equation}
...
\swp{Forgot words. Not sure if ``inverse-exponentially weighted'' is clear, however.}
\jd{I fiddled the words -- but the equation ref is the key here; I think it's fine.}
...
\jd{What's wrong with this reference (see rendering)?}
...
However, our simulations suggest that the initial exponential growth assumption still provides a useful approach for estimating the reproductive number.
...
Estimating generation-interval distributions is a key, and often under-appreciated, component of disease modeling and forecasting.
...
These predictable differences can arise from temporal effects, local spatial (or network) effects and population-level effects.
We have shown that correcting for temporal effects allows us to estimate a spatially informed distribution, which accurately describes how disease spreads in a population. 
Future studies should carefully consider how measurement influences estimated generation-interval distributions, and how these distributions influence spread of disease. 
