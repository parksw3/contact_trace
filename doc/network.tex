...
Then, the (yet to be realized) generation time between individual $i$ and the $k$-th contactee (during infectious stage $j$) is equal the sum of the length of latent period $E_i$ and the time between all events (either a contact or transistion between states) caused by this individual prior to the contact:
\begin{equation}
G_{i,j,k} = \begin{cases}
E_i + \sum_{n=1}^k T_{i, j, n}, & \text{if } j = 1,\\
E_i + \sum_{m=1}^{j-1} \sum_{n=1}^{C_{i, m} + 1} T_{i, m, n} + \sum_{n=1}^k T_{i, j, n}, & \text{if } j > 1.
\end{cases}
\end{equation}
Finally, the time at which this individual is contacted by individual $i$ is equal to the sum of current time $t$ and the generation time $G_{i,j,k}$.
...
Simulation continues until there are no more contacts are left in the queue.
