...
\end{equation}
where $f(a)$ represents a probability density over a (possibly multi-dimensional) aspect space.
\swp{Essentially, the population-level egocentric kernel further accounts for the probability that a susceptible individual has not been infected within an infected-susceptible pair.}.
...
\swp{we showed that these effects shorten the mean observed generation intervals, which in turn will reduce the estimate of the reproduction number \cite{wallinga2007generation, park2019practical}.}
...
The first person in the queue becomes infected \swp{(and therefore decreasing $S_i$ by 1 for all individuals $i$ that are acquaintances of the newly infected individual)}, and the current time is updated to infection time of this individual. 
