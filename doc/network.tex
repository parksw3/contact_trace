...
An epidemic can be characterized by its speed (exponential growth rate, $r$) and its strength, characterized by its reproductive number, \RR.
...
the time of infection of the infectors can range anywhere from the beginning of an epidemic to the most recent infection.
\dc{I found the previous paragraph difficult to read and edited it to try to improve. The `cohorts` were not referring to the same ones (I think) and that was confusing. The bckwrd GI is observed at one given time and those infectees each had infectors from potentially different cohorts.}
While an epidemic is ongoing, these measurements are ``censored'': we don't know what happens after the observation time.
These censored observations can be thought of as a weighted average of backward generation-interval distributions; \dc{weighted by what?}
...
This means that contacts made late in an individual's infection are more likely to be ``masked'' due to infectious contacts that were made earlier \dc{Although I get you try to explain in simple terms what's going on with ``masked'', I think it's not really clear: masked by what?  is it that subsequent contacts in the neighbourhood do not count given they were infected earlier?}.
...
An individual-level intrinsic infection kernel $K_a(\tau)$ \dc{is it clearer to specify the kernel is about infections?} describes the rate at which secondary infections are expected to be caused by an infected individual $a$.
Individuals may vary in their prognosis (e.g., duration of a latent and an infectious period) and their infectiousness, which can depend both on the intrinsic infectiousness of a disease as well as the contact structure around an infected individual.
...
\dc{wouldn't it be better to say: ``The population-level kernel describes the rate at which secondary infections are expected to be caused by an infected individual, on \emph{average}.'' I have troubles describing an ``average  infected individual''... Oh no, but you say it again below... well, I keep this comment, do whatever you want! }

There are two components to an infection kernel: intrinsic infectiousness of an average infected individual (the basic reproductive number)\dc{again, for me it's the intrinsic infectiousness averaged over the population, that's the $\int da$ thing. I stop from now on complaining about this ;)} and time distribution of new infections (generation-interval distribution).
...
\dc{again, weighted by what? the binary variable ``observed/not observed''? It's disturbing that we talk about weights but never say what they are...}
...
\end{equation}
As shown in \cite{champredon2015intrinsic} \dc{I added the reference for the reader who wants to see how exactly this is derived}, the backward generation-interval distribution, $b_t(\tau)$, describes a distribution of infection that occurred $\tau$ time units before the reference time $t$ and is proportional to $i(t-\tau) g(\tau)$:
\begin{equation}
b_t(\tau) = \frac{i(t-\tau) g(\tau)}{\int_0^t i(t-\sigma) g(\sigma) d\sigma}
\end{equation}
On the other hand, the censored generation-interval distribution, $c_t(\tau)$, describes a distribution of \emph{all} infections that are $\tau$ time units apart from any cohort infection time $s$ \dc{you mean:``[...] any cohort infected at time $s$[...]''?}  before $t$: $\int_0^t i_{s-\tau}(s) ds$.
Then, the censored generation-interval distribution is given by normalizing \dc{I felt we could explain a bit more, hence edited accordingly}
...
\end{equation}
This relationship shows that the backward generation-interval distribution is equivalent to the censored generation-interval distribution when $S \approx 1$, i.e., during the initial stage of the epidemic, when it is growing exponentially.
...
For a single outbreak, the observed mean \dc{shouldn't it be ``censored'' instead of ``observed mean''??} generation interval through contact tracing will always be shorter than the mean intrinsic generation interval (Figure~\ref{fig:censor}).
...
\dc{I'm confused... Figure~\ref{fig:censor} does not show they are the same. On the other hand, \autoref{eq:exp} is an equality... which one shall I believe?}
Second, decreasing number of susceptibles over the course of an epidemic makes long infections less likely to occur, and the realized \dc{censored?} intervals will always be shorter \citep{champredon2015intrinsic}.
...
The intrinsic generation-interval distribution that corresponds to this model can be written has \citep{Champredon2018equivalence} \dc{changed to a more appropriate reference (and self-promotion!)}:
\begin{equation}
...
When an epidemic is growing exponentially, the right-censoring causes the observed generation-interval inverse exponentially weighted \eref{exp}; \dc{the previous sentence doesn't read well; forgot words?}
...
\dc{Wait, I'm lost! what is the first method? what it the second method?!}
...
\dc{I'm confused... \autoref{eq:geff} shows the adjustment needed because of the censoring, that is clear. Is this what you call ``population-level''? But does the paragraph above provide also an adjustment?? The individual-level?? }

\dc{Oh OK, I'm back here after reading the Methods section. Please, give an explicit reference to that section, and also to the equations. As you could see with my comment above, I was lost, but having such references would have helped me followed... }
...
\dc{I suggest a short paragraph putting this work in perspective to other studies (like for example https://www.ncbi.nlm.nih.gov/pubmed/29087987). You also did it with Trapman, etc. throughout the main text, but it would be nice to summarize }
