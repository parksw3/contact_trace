...
First, we compared how the estimates from the two methods vary across time using a single stochastic simulation on a homogeneous network (Figure~\ref{fig:example}).
As expected, we find that using the observed generation-interval distributions always underestimates both the mean generation interval and the reproductive number;
due to initial overestimation of the growth rate, 
