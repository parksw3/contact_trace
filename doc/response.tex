...
Lines 42-43 refers to a single measurement, rather than a distribution. We have made this clear (see next comment).
...
We make clear that this definition corresponds to the definition at the individual-level (i.e., a single interval), which is unambiguous, and distinguish it from the population-level distributions:
...
We now define $g(\tau)$ as follows: ``expected time distribution of infectious contacts by a primary case''.

\revtext{
2. Is it the p.d.f. corresponding to the definition in Lines 42-43? Proof?
}

Definition in Lines 42-43 does not correspond to the definition of the intrinsic generation-interval distribution. 
It is a single measurement --- we tried to make it clear that it is not directly connected to any p.d.f.
We have tried to make this clearer in the text.
...
We no longer use the term effective generation-interval distribution.

\rev{Lines 241-243: "In a large homogeneously mixing population, the e§ective
...
\rev{However, I suggest the authors to compare the "individual-based
...
\rev{The authors has used the notations $\RR$ (Line 35) and $\Ro$ (Lines 98-99) for reproduction numbers and
linked them via $\RR = \Ro S(t)$:}

\rev{Shouldnít $\RR$ be a function of time $t$ via $S(t)$?}

We no longer use $\RR$ to avoid confusion. We now use $\Rini = \Ro S(0)$ to reflect the possibility that $S(0)$ may not be 1.

\rev{Is $S(t)$ a number or a proportion? In deterministic models, this matters
...
We now mention in Introduction that $\Ro$ can also affect the endemicity level and added a few appropriate citations.

\rev{$g(\tau)$ as a p.d.f. given by (2) and Line 101 is also an equilibrium
distribution.}

We disagree with the reviewer that the intrinsic generation-interval is an equilibrium distribution.
It is only an equilibrium distribution for a model that assumes a large, homogeneously mixing population.
...
\rev{ Under suitable assumptions on homogeneous mixing, the intrinsic
...
We agree that $g(\tau)$ measures the infectiousness within an infected individual without involving another individual.
We defined the intrinsic generation-interval distribution as follows:

``The \emph{intrinsic} generation-interval distributions describe the expected time distribution of infectious contacts made by a primary case.''

\revtext{The distribution $g(\tau)$ links the two intrinsic parameters R0 and r
(homogeneous mixing). Both $\Ro$ and $g(\tau)$ are intrinsic in the context of the equilibrium conditions. Therefore the parameter $r$ is a
theoretical value and is very di§erent from any observed exponential
or sub-exponential growth rate as the system is not at equilibrium.}

\rev{Observed exponential or sub-exponential growth events are different from the infection events. The current paper only addresses the infection events (mostly unobservable). This was
...
We point out in the Discussion that estimated $r$ can be different from the true $r$. 
However, it is not \emph{extremely} misleading to use the estimated $r$ from the observed data. 
See: 
...
$n \to \infty$'' 
This is inaccurate.
It is still possible to observe exponential growth when the population is not fully susceptible ($S(t) \approx S(0) \neq 1$) as long as the depletion of the susceptible population is negligible.
...
We disagree the statement that it is ``too strong'' to say that a more general distribution is more realistic than a special case.
To avoid confusion, we now say refer to Erlang distributions as ``more realistic approximations''.
...
The SEIR model is just an example --- the purpose of this paper is not to go through all possible generalizations of the SEIR model, but rather to explore spatial components.
The Laplace tranforms are only more useful for the intrinsic generation-interval distributions. 
The forward generation-interval distribution is analytically intractable and therefore, the Laplace transforms no longer hold.
...
In particular, the reviewer asks us to prove special cases of the Euler-Lotka equation, which has been proven numerous times as suggested by the references, using Laplace transforms; 
we do not think this is any relevant to the discussion of our paper.
We cite all Yan et al papers that were mentioned in the review in our main text.

\rev{
How to assess if the gamma distribution is a good choice as a
model? Run some kind of goodness-of-fit tests? How does the likelihood
...
``the inferred generation-interval distribution is then used to estimate the reproductive number $\Rini$ from the observed growth rate $r$ using the Euler-Lotka equation.''
...
We make clear that we do not want the intrinsic distribution. 
