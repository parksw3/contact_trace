...
\newtext{There are important distinctions to be made when defining generation-interval distributions at the population-level. The \emph{intrinsic} generation-interval distributions describe the expected time distribution of infectious contacts made by a primary case.
...
\newtext{While an epidemic is ongoing, realized generation intervals can be measured by identifying who infected whom and can be aggregated to form a single distribution.
...
\swp{??? What do you mean by ``also taken care to empirically validate our statistical approaches''}
...
We had initially thought their approach is similar due to their use of Poisson likelihood.
We realized that their approach is quite different from the goal of our paper and have removed the sentence.
They do not use Euler-Lotka or measure generation intervals.
...
The method of estimating the initial exponential growth rate by fitting a logistic curve has been previously validated against simulations of stochastic comparmental models \citep{ma2014estimating}.
}
